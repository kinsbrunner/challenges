...
ranges and returns an array of condensed ranges. For example, given:
...
Look at this case:
[(1, 10), (2, 6), (3, 5), (7, 9)]
Here all of our meetings should be merged together into just (1, 10). We need 
keep in mind that after we've merged the first two we're not done with the 
result—the result of that merge may itself need to be merged into other meetings 
as well.
Make sure that your function won't "leave out" the last meeting.
We can do this in O(n lg n) time.
We can tell the meetings overlap because the end time of the first meeting is 
AFTER or THE SAME AS the start of the second meeting. The first meeting here 
should be the meeting that starts first.
Be sure to consider these edge cases:
The end time of the first meeting and the start time of the second meeting are 
equal. For example: [(1, 2), (2, 3)]
...
class Meetings
  def self.condense_meetings(dates)
    merged_meetings = []
    sorted = dates.sort
    ...
    sorted[1..-1].each do |current_start, current_end|
      if current_start <= previous_end
        ...
        merged_meetings << ([previous_start, previous_end])
...
class TestMeetings < Minitest::Test
  def test_1
    assert_equal( Meetings.condense_meetings( [[0, 1], [3, 5], [4, 8], [10, 12], [9, 10]] ), [[0, 1], [3, 8], [9, 12]] )
  end

  def test_2
    assert_equal( Meetings.condense_meetings( [[1, 2], [2, 3]] ), [[1, 3]] )
  end
  
  def test_3
    assert_equal( Meetings.condense_meetings( [[1, 5], [2, 3]] ), [[1, 5]] )
  end
  
  def test_4
    assert_equal( Meetings.condense_meetings( [[1, 10], [2, 6], [3, 5], [7, 9]] ), [[1,10]] )
  end
  
  def test_5
    assert_equal( Meetings.condense_meetings( [[0, 1], [3, 5], [7, 9]] ), [[0, 1], [3, 5], [7, 9]] )
  end
end